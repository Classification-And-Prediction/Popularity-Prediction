...
for i in range(1,11):
	...
	logreg_model = LogisticRegression(penalty = 'l1',dual = False,C = value/10)
   
	#logreg_model = logreg_model.fit(X=X_train, y=y_train)

	#y_pred = logreg_model.predict(X_test)

	accu_score = cross_val_score(logreg_model,body_title_tags_corpus[0:len(train_data_df)],train_data_df.Popularity ,cv=10 ,scoring='accuracy').mean()
	#print "\n"
	#print "accuracy score : ",accu_score  
	accu_list.append(accu_score)

	precision_score = cross_val_score(logreg_model,body_title_tags_corpus[0:len(train_data_df)],train_data_df.Popularity,cv=10,scoring='precision').mean()
	#print "\n"
	#print "precision score : ",precision_score
	precision_list.append(precision_score)

	recall_score = cross_val_score(logreg_model,body_title_tags_corpus[0:len(train_data_df)],train_data_df.Popularity,cv=10,scoring='recall').mean()
	#print "\n"
	#print "recall score : ",recall_score
	recall_list.append(recall_score)

	f1_score = cross_val_score(logreg_model,body_title_tags_corpus[0:len(train_data_df)],train_data_df.Popularity,cv=10,scoring='f1').mean()
	#print "\n"
